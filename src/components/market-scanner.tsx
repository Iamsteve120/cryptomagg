    ...
    <>
      ...
      >
        <button
          ...
          aria-label={tr("Open AI market scanner")}
        ...
        <DialogContent className="max-h-[calc(100dvh-1rem)] w-[calc(100%-1rem)] max-w-xl gap-0 overflow-y-auto rounded-lg border-primary/30 bg-card p-0 sm:max-h-[calc(100dvh-2rem)] sm:w-[calc(100%-2rem)]">
          <DialogHeader className="sticky top-0 z-10 border-b border-border bg-card px-4 py-4 text-left sm:px-5 sm:py-5">
            <div className="flex items-center gap-3">
              ...
              <div><DialogTitle>{tr("Entry scanner")}</DialogTitle><DialogDescription>{tr("AI analysis across every supported market")}</DialogDescription></div>
            </div>
          </DialogHeader>
          ...
          <div className="space-y-4 px-4 pb-4 pt-4 sm:px-5 sm:pb-5">
            {!result && !scanMutation.isPending ? (
              <div className="space-y-4 py-2">
                <div className="rounded-md border border-border bg-secondary/25 p-4"><p className="font-medium">{tr("Scan all supported markets")}</p><p className="mt-1 text-sm text-muted-foreground">Compares momentum, daily range, direction, and liquidity, then ranks every market so you can pick your own setup.</p></div>
                <Button className="w-full" onClick={() => scanMutation.mutate()}><Radar className="size-4" /> {tr("Scan markets")}</Button>
              </div>
            ) : null}

            {scanMutation.isPending ? (
              <div className="py-10 text-center"><LoaderCircle className="mx-auto size-8 animate-spin text-primary" /><p className="mt-4 font-semibold">{tr("Analyzing live markets")}</p><p className="mt-1 text-sm text-muted-foreground">{tr("Comparing price action and liquidity across every supported asset.")}</p><div className="mx-auto mt-5 h-1.5 max-w-xs overflow-hidden rounded-full bg-secondary"><motion.div className="h-full bg-primary" initial={{ width: "8%" }} animate={{ width: "86%" }} transition={{ duration: 12, ease: "easeOut" }} /></div></div>
            ) : null}
            ...
            {result && active && !scanMutation.isPending ? (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-md border border-border p-3"><p className="text-xs text-muted-foreground">{tr("Confidence")}</p><p className="num mt-1 text-lg font-semibold">{active.confidence}%</p></div>
                  <div className="rounded-md border border-border p-3"><p className="text-xs text-muted-foreground">{tr("Expiry")}</p><p className="num mt-1 text-lg font-semibold">{active.durationSeconds < 60 ? `${active.durationSeconds}s` : `${active.durationSeconds / 60}m`}</p></div>
                </div>

                <div>
                  <p className="text-xs font-semibold uppercase text-muted-foreground">{tr("All market options")}</p>
                ...
                <div><p className="text-xs font-semibold uppercase text-muted-foreground">{tr("Market condition")}</p><p className="mt-1 text-sm">{result.marketCondition}</p></div>
                <div><p className="text-xs font-semibold uppercase text-muted-foreground">{tr("Why this setup")}</p><p className="mt-1 text-sm leading-6">{result.rationale}</p></div>
                ...
                <div><Label htmlFor="scannerStake">{tr("Demo stake")}</Label><Input id="scannerStake" className="mt-2" type="number" inputMode="decimal" min="0.5" max="500" step="1" value={stake} onChange={(event) => setStake(event.target.value)} /><p className="mt-1 text-xs text-muted-foreground">Minimum 0.5 USD | Maximum 500 USD | Available {balance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USD</p></div>
                <div className="grid gap-2 sm:grid-cols-2"><Button variant="outline" onClick={() => scanMutation.mutate()} disabled={scanMutation.isPending}><Radar className="size-4" /> {tr("Scan again")}</Button><Button className="min-h-10 whitespace-normal" disabled={!canExecute} onClick={executeResult}><ChartNoAxesCombined className="size-4 shrink-0" />{mode === "live" ? "Demo scanner only" : `Place Demo trade on ${active.symbol}`}</Button></div>
              </div>
            ) : null}
