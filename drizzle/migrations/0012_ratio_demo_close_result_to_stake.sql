...
BEGIN
  ...
  -- Minimum simulated result scales with the trade amount (1 USD stake -> 0.46 USD).
  base_result := ROUND(target_trade.stake*0.46, 2);
  ...
  END IF;
  IF live_pnl = 0 THEN
    IF random() < 0.95 THEN live_pnl := GREATEST(base_result, tp_amount); ELSE live_pnl := 0 - LEAST(sl_amount, base_result); END IF;
  ELSIF live_pnl > 0 THEN
    live_pnl := GREATEST(live_pnl, LEAST(tp_amount, base_result));
  ELSE
