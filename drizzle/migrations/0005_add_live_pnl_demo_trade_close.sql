CREATE OR REPLACE FUNCTION public.close_demo_trade_at_live_pnl(
  p_user_id UUID,
  p_trade_id UUID,
...
DECLARE
  ...
  live_pnl NUMERIC(18,2);
  credit NUMERIC(18,2);
  result_status TEXT;
BEGIN
  IF p_exit_price IS NULL OR p_exit_price <= 0 THEN
    RAISE EXCEPTION 'Invalid live price';
  END IF;
  ...
  IF target_trade.direction = 'up' THEN
    movement := p_exit_price - target_trade.entry_price;
  ELSE
    movement := target_trade.entry_price - p_exit_price;
  END IF;

  IF movement >= 0 THEN
    target_distance := ABS(target_trade.take_profit_price - target_trade.entry_price);
    IF target_distance IS NULL OR target_distance = 0 THEN
      live_pnl := 0;
    ELSE
      live_pnl := ROUND(
        LEAST(1, movement / target_distance) * target_trade.stake * target_trade.payout_rate / 100,
        2
      );
    END IF;
  ELSE
    ...
    ELSE
      live_pnl := ROUND(
        0 - LEAST(1, ABS(movement) / target_distance) * target_trade.stake,
        2
      );
    END IF;
  END IF;
