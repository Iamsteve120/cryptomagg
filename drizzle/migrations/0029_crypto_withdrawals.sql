...
CREATE OR REPLACE FUNCTION public.hold_crypto_withdrawal(p_user_id uuid, p_amount numeric, p_asset text, p_network text, p_address text)
RETURNS SETOF public.crypto_withdrawals LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE current_balance numeric(18,2); created public.crypto_withdrawals; transaction_uuid uuid;
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.withdrawal_requests WHERE user_id = p_user_id AND status = 'pending')
     OR EXISTS (SELECT 1 FROM public.crypto_withdrawals WHERE user_id = p_user_id AND status = 'pending')
  THEN RAISE EXCEPTION 'Pending withdrawal exists'; END IF;
  ...
  VALUES (p_user_id, 'withdrawal', 'crypto', p_asset, p_amount, 'pending', 'live', p_address) RETURNING id INTO transaction_uuid;
  INSERT INTO public.crypto_withdrawals(user_id, amount_usdt, asset, network, destination_address, transaction_id)
  VALUES (p_user_id, p_amount, p_asset, p_network, p_address, transaction_uuid) RETURNING * INTO created;
  RETURN NEXT created;
END; $$;

CREATE OR REPLACE FUNCTION public.finalize_crypto_withdrawal(p_request_id uuid, p_provider_id text, p_success boolean, p_tx_hash text DEFAULT NULL, p_failure_reason text DEFAULT NULL)
RETURNS boolean LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE target public.crypto_withdrawals;
BEGIN
  SELECT * INTO target FROM public.crypto_withdrawals
  WHERE (p_request_id IS NOT NULL AND id = p_request_id) OR (p_provider_id IS NOT NULL AND provider_payout_id = p_provider_id)
  ORDER BY created_at DESC LIMIT 1 FOR UPDATE;
  IF target.id IS NULL OR target.status <> 'pending' THEN RETURN false; END IF;
  IF p_success THEN
    UPDATE public.crypto_withdrawals SET status = 'paid', tx_hash = NULLIF(p_tx_hash,''), updated_at = now() WHERE id = target.id;
    UPDATE public.transactions SET status = 'completed' WHERE id = target.transaction_id;
  ELSE
    UPDATE public.profiles SET live_balance = live_balance + target.amount_usdt, updated_at = now() WHERE id = target.user_id;
    UPDATE public.crypto_withdrawals SET status = 'rejected', failure_reason = LEFT(COALESCE(NULLIF(p_failure_reason,''),'provider_error'),200), updated_at = now() WHERE id = target.id;
