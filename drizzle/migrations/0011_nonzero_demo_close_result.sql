...
AS $function$
DECLARE target_trade public.trades; current_balance NUMERIC(18,2); movement NUMERIC; target_distance NUMERIC; live_pnl NUMERIC(18,2); credit NUMERIC(18,2); result_status TEXT; sensitivity NUMERIC := 100; tp_amount NUMERIC(18,2); sl_amount NUMERIC(18,2);
BEGIN
  ...
  END IF;
  -- Never settle a closed Demo trade at exactly zero: fall back to the simulated practice outcome.
  IF live_pnl = 0 THEN
    IF random() < 0.95 THEN
      live_pnl := GREATEST(0.01, ROUND(tp_amount*(0.25+random()*0.75),2));
    ELSE
      live_pnl := 0 - GREATEST(0.01, ROUND(sl_amount*(0.25+random()*0.75),2));
    END IF;
  END IF;
